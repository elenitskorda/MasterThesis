...
data <- read.delim("OvCa phosphoProteins-DE_Normalised.tsv.phosphosites.tsv", sep = "\t", check.names = FALSE)
data$Genes <- paste(data$Genes, paste(data$residue, data$Phosphosite, sep = ""), sep = "_")
num_duplicates <- sum(duplicated(data$Genes))
...
# 
# # Choose the filled data frame with fewer NAs in each row
...
peptide <- read.delim("abundance_single-site_MD.tsv", sep = "\t", check.names = FALSE)
# Assuming your data frame is named 'data'
peptide$Gene <- paste(peptide$Gene, sub(".*_", "", peptide$Index), sep = "_")

peptide <- peptide[, !(names(peptide) %in% c("Index", "ProteinID", "Peptide", "SequenceWindow" ,"MaxPepProb", "ReferenceIntensity"))]
Gene = peptide[, 1]
...
#cols_to_remove <- c("01OV029", "010OV17","01OV007.1","15OV001","01OV039.1","11OV002.1","14OV011.1","01OV023.1","17OV002","17OV001.1")
...
# Let's say 'df' is your data frame and you want to remove columns 'col1' and 'col2'
#cols_to_remove <- c("01OV029", "01OV017","01OV007.1","15OV001","01OV039.1","11OV002.1","14OV011.1","01OV023.1","17OV002","17OV001.1")
...
sdata <- FindNeighbors(sdata, dims = 1:5)
...
anchors <- FindTransferAnchors(reference = sdata, query = peptides, dims = 1:5, 
                               reference.reduction = "pca",k.anchor=11)
predictions <- TransferData(anchorset = anchors, refdata = sdata$Cell_type, dims = 1:5, k.weight = 7)
...
pancreas.ref <- RunUMAP(sdata, dims = 1:5, return.model = TRUE)
...
pancreas.query <- MapQuery(anchorset = anchors, reference = pancreas.ref, query = pancreas.query, refdata = list(predicted_celltype = "Cell_type"),
                           reference.reduction = "pca", reduction.model = "umap", transferdata.args = list(k.weight = 7))
...
plot=p1+p3+p2+plot_annotation(title = "Phospo_VS_Phospo on Phosphosite Level")  &  theme(plot.title = element_text(hjust = 0.5,size = 15)) 
plot
...
P1=DoHeatmap(pancreas.query,features = top10$gene,group.bar = TRUE,label=TRUE,angle=30,size = 3.5,draw.lines = TRUE)+  guides(colour=F)
P1
# P2=DoHeatmap(pancreas.query, features = top10$gene,group.by = "Cells",group.bar = TRUE,label=TRUE,size = 2.6,angle=90)+  guides(colour=FALSE)
# P2
# P1+P2
...
head(x = all.markers2)
top10 <- all.markers2 %>% group_by(cluster) %>% top_n(30, avg_log2FC)
all.markers2 %>%
  group_by(cluster) %>%
...
# p4=DimPlot(refquery, group.by = 'id',shuffle = TRUE)+ggtitle("Reference and Query annotations")
# plot2=p1+p3+p4+p2+plot_annotation(title = "Phospo_VS_Phospo on Phosphosite Level")  &  theme(plot.title = element_text(hjust = 0.5,size = 15)) 
# plot2
# 
# 
# 
