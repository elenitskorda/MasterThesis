...
data <- read.delim("OvCa Proteins 5sub Loess DE_stats.tsv", sep = "\t", check.names = FALSE)

# Remove unnecessary columns from the data
data = subset(data, select = -c(1:3))
data = subset(data, select = -c(2:12))
...
peptide <- read.delim("abundance_gene_MD_18.tsv", sep = "\t", check.names = FALSE)
peptide <- peptide[, !(names(peptide) %in% c("NumberPSM", "ProteinID", "Peptide", "MaxPepProb", "ReferenceIntensity"))]
Gene = peptide[, 1]
...
anchors <- FindTransferAnchors(reference = sdata, query = peptides, dims = 1:6, reference.reduction = "pca",k.anchor=10)
predictions <- TransferData(anchorset = anchors, refdata = sdata$Cell_type, dims = 1:6, k.weight = 6)
...
# P3=DoHeatmap(pancreas.query,features = all.markers$gene,group.bar = TRUE,label=TRUE,angle=30,size = 3.5)+  guides(colour=FALSE)
# P3
...
#           p.val.cutoff = 0.05,
#           cols = NULL,
#           enrich.database = "ARCHS4_Cell-lines",
#           num.pathway = 30,
#           return.gene.list = F)
